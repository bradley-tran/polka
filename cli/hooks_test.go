...
func TestDefaultCLIHookRegistryLifecycleOrder(t *testing.T) {
	...
	if got, want := statusHookIDs(registry.runtimeStatusHooks), []string{"webserver", "phpmyadmin", "database", "mailpit"}; !reflect.DeepEqual(got, want) {
...
func TestDefaultCLIHookRegistryConfigStatusOrder(t *testing.T) {
	...
	want := []string{"php", "composer", "pie", "nodejs", "mago", "nginx", "apache", "frankenphp", "sqlite", "phpmyadmin", "database", "mailpit"}
