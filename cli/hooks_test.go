...
func TestDefaultCLIHookRegistryLifecycleOrder(t *testing.T) {
	...
	if got, want := startServiceHookIDs(registry.startServices), []string{"database", "mailpit", "meilisearch", "phpmyadmin"}; !reflect.DeepEqual(got, want) {
	...
	if got, want := stopHookIDs(registry.stopHooks), []string{"webserver", "phpmyadmin", "meilisearch", "database", "mailpit"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("stop hooks = %#v, want %#v", got, want)
	}
	if got, want := statusHookIDs(registry.runtimeStatusHooks), []string{"webserver", "phpmyadmin", "meilisearch", "database", "mailpit"}; !reflect.DeepEqual(got, want) {
...
func TestDefaultCLIHookRegistryConfigStatusOrder(t *testing.T) {
	...
	want := []string{"php", "composer", "pie", "nodejs", "mago", "nginx", "apache", "frankenphp", "sqlite", "meilisearch", "phpmyadmin", "database", "mailpit"}
