...
func redisAddress(state redisRuntimeState) string {
	return service.RedisAddress(state.Port)
}

func redisAddressForConfig(redis *backend.RedisConfig) string {
	return service.RedisAddressForConfig(redis)
}
