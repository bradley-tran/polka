...
func (config PHPInstallConfig) IsZero() bool {
	return len(config.Extensions) == 0 && len(config.ZendExtensions) == 0 && strings.TrimSpace(config.MemoryLimit) == "" && len(config.OPcacheConfig) == 0 && strings.TrimSpace(config.CABundlePath) == ""
}
...
func EffectivePHPConfigForInstall(environment config.Environment) PHPInstallConfig {
	return PHPInstallConfig{
		...
		ZendExtensions: config.NormalizePHPExtensions(environment.ZendExtensions),
...
func renderPHPConfig(extensionDir string, phpConfig PHPInstallConfig) ([]byte, error) {
	...
	builder.WriteString("; Generated by Polka. Re-run polka install after editing php-extensions, pecl-extensions, memory-limit, or OPcache config.\n")
	...
	for _, name := range names {
		if isZendPHPExtension(name) || phpConfig.ZendExtensions[strings.ToLower(strings.TrimSpace(name))] {
