...
func TestDefaultRegistryValidatesRedisConfig(t *testing.T) {
	...
	if err := registry.ValidateEnvironment(config.Environment{Redis: &config.RedisConfig{Version: "8.8", Port: 70000}}); err == nil {
	...
	environment := config.Environment{Redis: &config.RedisConfig{Version: "8.8"}}
	active := registry.ActiveCommandNames(&environment)
	if !reflect.DeepEqual(active, []string{"redis-server", "redis-cli"}) {
		t.Fatalf("ActiveCommandNames(redis env) = %#v, want redis commands", active)
	}
}
