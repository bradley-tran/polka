...
// RedisServerSpec describes a Redis process launch.
...
func EnsureManagedRedisStarted(ctx Context, hooks RedisRuntimeHooks) (RedisRuntimeState, bool, error) {
	...
	startedState := RedisRuntimeState{
		...
		AuthEnabled:     strings.TrimSpace(spec.Password) != "",
		PasswordHash:    RedisPasswordHash(spec.Password),
...
func RedisServerArgs(spec RedisServerSpec) []string {
	...
	if strings.TrimSpace(spec.Password) != "" {
		args = append(args, "--requirepass", strings.TrimSpace(spec.Password))
	}
...
func RedisURL(state RedisRuntimeState) string {
	return RedisURLForConfig(&config.RedisConfig{Port: state.Port})
}

func RedisURLForConfig(redis *config.RedisConfig) string {
	if redis == nil {
		return ""
	}

	return "redis://" + RedisAddress(EffectiveRedisPort(redis))
}
...
func RedisStateMatchesEnvironment(state RedisRuntimeState, environment config.Environment) bool {
	...
	return strings.TrimSpace(state.Version) == strings.TrimSpace(environment.Redis.Version) &&
		...
		state.AuthEnabled == (strings.TrimSpace(environment.Redis.Password) != "") &&
		strings.TrimSpace(state.PasswordHash) == RedisPasswordHash(environment.Redis.Password)
}
...
func RedisPasswordHash(password string) string {
